document.addEventListener("DOMContentLoaded", () => {
  ...
  firebase.auth().onAuthStateChanged(async (user) => {
    ...
    try {
      ...
      if (userDataSnapshot.exists()) {
        ...
        if (
          userData.skill === "dsa" ||
          userData.skill === "full-stack-development"
        ) {
  ...
  startTest.addEventListener("click", () => {
    ...
    document.getElementById("quiz-title").textContent =
      selectedSkill === "dsa"
        ? "DSA (Data Structures & Algorithms) Test"
    ...
    const quizzes = {
      dsa: {
        questions: [
          ...
          {
            question:
              "Which data structure is best suited for implementing a recursive function stack?",
            options: ["Queue", "Linked List", "Array", "Stack"],
            correct: 3,
          },
          {
            question:
              "Which traversal method visits left subtree, root, then right subtree?",
            options: ["Preorder", "Inorder", "Postorder", "Level Order"],
            correct: 1,
          },
          {
            question:
              "In which case does a binary search tree (BST) become inefficient like a linked list?",
            options: [
              "When balanced",
              "When it has both left and right children",
              "When nodes are inserted in sorted order",
              "When height is minimal",
            ],
            correct: 2,
          },
          {
            question:
              "What is the space complexity of a recursive implementation of binary search?",
      ...
      "full-stack-development": {
        questions: [
          ...
          },
          {
            ...
            options: [
              "SQL is schema-less and NoSQL is strictly structured",
              "SQL is best for unstructured data",
              "SQL uses fixed schemas; NoSQL offers flexible structures",
              "NoSQL can't store relationships",
            ],
            correct: 2,
          },
          {
            question:
              "What happens in the componentDidMount() method in React?",
            options: [
              "Component is destroyed",
              "Component is updated",
              "DOM is created and API calls are made",
          ...
          },
          {
            ...
            options: [
              "By hashing the password and comparing it to a key",
              "By encoding user info in a token sent with requests",
              "By opening a socket connection",
              "By verifying cookies",
            ],
            correct: 1,
          },
          {
            ...
            options: [
              "Only to handle routes",
              "To set database schemas",
              "To intercept and process requests before they reach routes",
          ...
          {
            question:
              "Which HTTP method should be used for idempotent operations?",
            options: ["POST", "PUT", "PATCH", "DELETE"],
            correct: 1,
          },
          {
            question:
              "What is the main benefit of using Docker in development?",
            options: [
          ...
          {
            question:
              "Which database indexing strategy is best for frequent range queries?",
    ...
    function displayQuestion() {
      ...
      // Hide Next Question button since it's not needed
  ...
  async function handleTestCompletion(score, totalQuestions) {
    ...
    try {
      ...
      await firebase.database().ref("quiz_results").push(testData);
      ...
      // If passed, show success modal and roadmap, then redirect to dashboard
      if (percentage >= 70) {
        ...
        const skill = testSkill.value;
        const roadmapPrompt = `The user scored ${score}/${totalQuestions} in a ${skill} quiz. Generate a personalized learning roadmap and skill reference for this user.`;
        try {
          const roadmap = await sendToGeminiAPI(roadmapPrompt);
          showRoadmapToUser(roadmap);
        } catch (err) {
          showRoadmapToUser(
            "Sorry, we couldn't generate a roadmap at this time."
          );
        }
  ...
  // Helper to show roadmap in modal or results section
  function showRoadmapToUser(roadmap) {
    // You can customize this to show in a modal, results section, etc.
    ...
    if (!roadmapContainer) {
      ...
      roadmapContainer.style.marginTop = "24px";
      roadmapContainer.style.background = "#23232a";
      roadmapContainer.style.color = "#fff";
      roadmapContainer.style.padding = "16px";
      roadmapContainer.style.borderRadius = "12px";
      results.appendChild(roadmapContainer);
    }
    roadmapContainer.innerHTML = `<h3>Your Personalized Roadmap</h3><p>${roadmap}</p>`;
  }
  ...
  const initialPrompts = [
    "Welcome! Let's start with a few questions. What's your name?",
    "What kind of skill are you looking to learn or which skill interests you most?",
  ...
  async function sendToGeminiAPI(message) {
    // Call your backend endpoint that proxies Gemini API
  ...
  chatSend.addEventListener("click", async () => {
    ...
    // Send user message to Gemini API and display bot reply
    const botReply = await sendToGeminiAPI(userMsg);
    displayBotMessage(botReply);

    chatStep++;
    if (chatStep < initialPrompts.length) {
      displayBotMessage(initialPrompts[chatStep]);
    } else {
      // Hide chat section and show quiz
  ...
  // Check for skill and level in localStorage
  const skill = localStorage.getItem("selectedSkill");
  const level = localStorage.getItem("selectedLevel");

  if (skill && level) {
    askGemini(
      `The user chose ${skill} at ${level} level. Generate 3 assessment questions.`
    );
  }
});
